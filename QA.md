...
Mobile editing-loop playback — September 24, 2026: Added explicit muted/defaultMuted/playsInline properties, native autoplay markup, visible video/poster rendering, and an accessible Play animation control when autoplay is rejected or reduced-motion is enabled. Explicit playback supports pause/resume and still pauses when offscreen or the page is hidden. Observes the visible scene instead of a potentially hidden video. Created a fast-start MP4 by lossless remux: all 192 decoded frames match the original eight-second loop. Added byte-range/HEAD support to the local preview server and checked partial, suffix, and unsatisfiable ranges. Chromium mobile tests passed autoplay, simulated NotAllowedError, reduced-motion opt-in, manual pause/resume, and scrolling out/back. Windows WebKit played the original live file but did not decode either version through localhost, including range delivery; live verification is required. Syntax checks and build passed. Actual phone acceptance remains pending.
